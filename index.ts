...
console.log(
  chalk.yellowBright(
    `\n Dear ${chalk.whiteBright(name)}! You have ${chalk.greenBright(
      formatTime(input)
    )} seconds to blast a bomb\n`
  )
);
...
const countdown = async () => {
  ...
  process.stdin.on("keypress", (char, key) => {
    if (key && key.name === "up") {
      ...
      console.log(
        chalk.greenBright(
          `\n Dear ${chalk.whiteBright(
            name
          )}! Countdown is paused, bomb is defused...\n`
        )
      );
      ...
      console.log(
        chalk.greenBright(
          `\n Dear ${chalk.whiteBright(
            name
          )}! thank you for using ${chalk.cyanBright(
            Developer
  ...
  while (remainingTime > 0) {
    if (!paused) {
      process.stdout.write(
        chalk.blueBright(
          `\r\tRemaining time: ${chalk.redBright(formatTime(remainingTime))}\n`
        )
      );
